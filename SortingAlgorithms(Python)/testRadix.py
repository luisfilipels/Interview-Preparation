...
#INICIO PIOR CASO
for x in numeros:
    tempoPior.append(timeit.timeit("aux={}\naux = radixSort(aux)\n".format(vectPior[0:x]), setup, number=1))
    passoPior.append(x)
...
#INICIO MELHOR CASO
for x in numeros:
    tempoMelhor.append(timeit.timeit("aux={}\naux = radixSort(aux)\n".format(vectMelhor[0:x]), setup, number=1))
...
#INICIO CASO NORMAL
for x in numeros:
    tempo.append(timeit.timeit("aux={}\naux = radixSort(aux)\n".format(vect[0:x]), setup, number=1))
    passo.append(x)
    print("Elementos: {}".format(x))
...
for lista in listas:
    casoTempo.append(
        timeit.timeit("aux={}\naux = radixSort(aux)".format(list(lista).copy()), setup,
                      number=1))
...
desenhaTresGraficos(passoMelhor, tempoMelhor, passoPior, tempoPior, passo, tempo, x1="Tempo", y1="Elementos", titulo="Radix Sort")