...
#INICIO PIOR CASO
for x in numeros:
    tempoPior.append(timeit.timeit("selectionSort({})".format(vectPior[0:x].copy()), setup, number=1))
    passoPior.append(x)
...
casoLista = []
for lista in listas:
    casoTempo.append(timeit.timeit("selectionSort({})".format(list(lista).copy()), setup, number=1))
